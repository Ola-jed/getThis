<form action="{{ url('/profile') }}" method="post" id="profile-update" class="box has-background-dark has-text-white column">
    ...
    <div class="field column">
        ...
        <input type="text" name="name" class="input is-primary" value="{{ session()->get('user')->name }}" maxlength="25" required>
        @error('name')
    ...
    <div class="field column">
        ...
        <input type="email" name="email" class="input is-primary" value="{{ session()->get('user')->email }}" required>
        @error('email')
            <div class="help is-danger">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="field column">
        ...
        <input type="password" name="new_password_confirm" class="input is-primary" >
